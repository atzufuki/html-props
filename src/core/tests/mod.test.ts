...
Deno.test("HTMLPropsMixin: event listener via prop with event config", () => {
  class TestElement extends HTMLPropsMixin(HTMLElement, {
    // deno-lint-ignore no-explicit-any
    onCustomEvent: prop<((e: Event) => void) | undefined>(undefined, {
      event: "custom-event",
    }),
  }) {
    render() {
...
Deno.test("HTMLPropsMixin: increment button pattern - internal signal change triggers re-render", () => {
  ...
  assertEquals(
    ...
    "Count: 1",
    "Button text should update to 1",
  );
  ...
  assertEquals(
    ...
    "Count: 2",
    "Button text should update to 2",
  );
...
Deno.test("HTMLPropsMixin: nested component - child mounted via parent render() has working reactivity", () => {
  ...
  const child = parent.querySelector("nested-counter-test") as InstanceType<
    typeof NestedCounter
  >;
  ...
  assertEquals(
    childRenderCount,
    2,
    "Child should re-render after internal signal change",
  );
